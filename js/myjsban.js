...
var initCheck = function(star, last) {
		...
		initCheckTop();
		initChangePage();
		$("#table_id").append("<div class='backMain'onclick='initContent(0,5)'>返回列表</div>");
			...
			if(i >= star && i < last) {
				...
				for(var j = 0; j < userArray[i].length - 1; j++) {
					...
					if(userArray[i][2].length > 3) {
						...
						//名称缩略						var str=userArray[i][j].substr(25);
						userArray[i][2] = userArray[i][2].replace(userArray[i][2].substr(2), "...");s
						//						userArray[i][j].length
					}
					//					div背景颜色
...
function changePageBackgroundColor(nowPageNumber) {
	if(userArray.length / pagelenth * 10 % 10 == 0) {
		for(var i = 1; i < userArray.length / pagelenth; i++) {
			if(i == nowPageNumber) {
				//				document.getElementById('changePageBtn'+nowPageNumber).('class','changePageBtnColor');

				$("#changePageBtn" + nowPageNumber).addClass("changePageBtnColor");
			} else {
				//				document.getElementById('changePageBtn'+i).removeAttribute('class','changePageBtnColor');
...
//			}
			...
			if(searchArray[i][2].length > 3) {
				...
				//										var str=userArray[i][j].substr(25);
...
function outWindow(values) {
	...
	var out_window = $(".out_window");
	$(".back_alph").css("z-index", "999");
	out_window.css("z-index", "1000");
	out_window.append("<div class='weith_100 look_boderdown_blu main_top_top'>信息详情</div>");
	for(var i = 0; i < userArray.length; i++) {
		if(i > 6) {

		} else {
			out_window.append("<div class='weith_30'>" + types[i + 3] + "</div>");
			out_window.append("<div class='weith_50 text_left'>" + userArray[values][i + 2] + "</div>");

		}
	}

	out_window.append("<div class='out_window_btn weith_100'><div class='background_blu font_color_gra panding_btn look_bodergay' onclick='backwindow()'id='back_main'>返回</div></div>");
	//	$(".out_window div").last().attr('style','margin-top: 1rem;');
}
...
function outTips(vals){
	$(".out_window div").remove();
	$(".out_window").attr("style",
	"z-index: 1000; width: 25rem;height: 10rem;left: 30rem; border: solid 5px cornflowerblue;");
	$(".out_window").append("<div>提示</div>");
	$(".out_window").append("<div>"+vals+"</div>");
	$(".out_window div").eq(0).attr("style","width: 100%;height: 2rem;border-bottom: 2px solid cornflowerblue;")
	$(".out_window div").eq(1).attr("style","width: 50%;height: 2rem;margin-left: 2rem;")
	setTimeout("backTips();",2000);
}
function backTips(){
	$(".out_window").attr("style","z-index:-1;");
	console.log("backTips");
}
